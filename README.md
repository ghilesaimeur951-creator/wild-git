...
Le test autonome `EngineCheck.java` est exécuté avant la compilation dans GitHub Actions. Les programmes, alarmes et historiques sont enregistrés uniquement sur l’appareil. Le widget utilise un programme fixe de 4 tours (5 s de préparation, 30 s d’effort, 10 s de repos). L’application ne demande pas d’accès aux contacts, au microphone ou à la position.
